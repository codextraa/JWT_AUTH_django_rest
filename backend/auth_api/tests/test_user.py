...
class RefreshTokenViewTests(APITestCase):
    def setUp(self):
        self.client = APIClient()
        self.refresh_url = (
            TOKEN_REFRESH_URL  # Assuming you named your URL 'token_refresh'
        )
        ...
        self.user = create_user(
            email=self.email, password=self.password, is_email_verified=True
        )
    ...
    def test_invalid_refresh_token(self):
        ...
        )  # Or 401 depending on your exact error handling
        self.assertIn("error", response.data)
    ...
    def test_refresh_token_for_social_auth_user(self):
        ...
        self.assertEqual(
            response.data["error"],
            f"This process cannot be used, as user is created using google",
        )
...
class LogoutViewTests(APITestCase):
    ...
    def test_logout_blacklist_failure(self):
        ...
        """
        with patch.object(
            RefreshToken, "blacklist", side_effect=Exception("Blacklist failed")
        ):
            response = self.client.post(
                self.url, {"refresh": self.refresh_token}, format="json"
            )
