...
export function validateSessionData(
  ...
  const user_id = String(data.user_id).trim(); // Convert to string and remove extra spaces

  // Validate user_role (should be a non-empty string)
  if (typeof data.user_role !== "string" || data.user_role.trim() === "") {
    return null;
  }
  const user_role = data.user_role.trim();
  ...
  const access_token_expiry = data.access_token_expiry.trim();
...
export function validateCSRFTokenData(
  ...
  const csrf_token = String(data.csrf_token).trim();

  if (typeof data.csrf_token_expiry !== "string") {
    return null;
  }
  const csrf_token_expiry = String(data.csrf_token_expiry).trim();
